...
docker-compose -f "${DIR}/devtools/docker-compose.yaml" run --rm py3.5.4 ./devtools/bin/uv-check.sh ${MODULE} 3.5
if [ $? -ne 0 ]; then
    echo "FAILED"
    exit 1
fi

docker-compose -f "${DIR}/devtools/docker-compose.yaml" run --rm py3.6.2 ./devtools/bin/uv-check.sh ${MODULE} 3.6
if [ $? -ne 0 ]; then
    echo "FAILED"
    exit 1
fi

docker-compose -f "${DIR}/devtools/docker-compose.yaml" run --rm py2.7.10 ./devtools/bin/uv-check2.sh
if [ $? -ne 0 ]; then
    echo "FAILED"
    exit 1
fi

docker-compose -f "${DIR}/devtools/docker-compose.yaml" run --rm py2.7.10 bash test/ansible/sanity/integration-test-idempotent-names.sh
if [ $? -ne 0 ]; then
    echo "FAILED"
    exit 1
fi
