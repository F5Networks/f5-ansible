...
.. raw:: html
    ...
    <td></td>
        <td></td>
        <td><div>Specifies the name of the device that you want to add to the device group. Often this will be the hostname of the device. This member must be trusted by the device already. Trusting can be done with the <code>bigip_device_trust</code> module and the <code>peer_hostname</code> option to that module.</div>        </td></tr>
    ...
    <td>present</td>
        ...
        <td><div>When <code>present</code>, ensures that the device group member.</div><div>When <code>absent</code>, ensures the device group member is removed.</div>        </td></tr>
                <tr><td>user<br/><div style="font-size: small;"></div></td>
    <td>yes</td>
    <td></td>
        <td></td>
