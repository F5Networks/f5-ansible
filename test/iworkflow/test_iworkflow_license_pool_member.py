...
from library.iworkflow_license_pool_member import (
    ArgumentSpec,
    ModuleManager,
    Parameters
)
...
class TestParameters(unittest.TestCase):
    ...
    def test_module_api_devices(self, *args):
        devices = load_fixture('load_license_pool_members.json')
        arguments = dict(
            devices=devices
        )
        p = Parameters(arguments)
        assert len(p.devices) == 1
        assert 'deviceReference' in p.devices[0]
        assert 'managementAddress' in p.devices[0]
        assert 'hostname' in p.devices[0]
        assert 'selfLink' in p.devices[0]
        assert 'address' in p.devices[0]
