...
class F5BaseClient(object):
    ...
    def merge_provider_params(self):
        ...
        if self.check_key('server', provider):
            result['server'] = provider['server']
        elif self.check_key('server', self.params):
            result['server'] = self.params['server']
        elif self.check_key('F5_SERVER', os.environ):
            result['server'] = os.environ['F5_SERVER']
        else:
            raise F5ModuleError('Server parameter cannot be None or missing, please provide a valid value')  # maybe different wording of the message

        if self.check_key('server_port', provider):
            result['server_port'] = provider['server_port']
        elif self.check_key('server_port', self.params):
            result['server_port'] = self.params['server_port']
        elif self.check_key('F5_SERVER_PORT', os.environ):
            result['server_port'] = os.environ['F5_SERVER_PORT']
        else:
            result['server_port'] = 443

        if self.check_key('validate_certs', provider):
            result['validate_certs'] = provider['validate_certs']
        elif self.check_key('validate_certs', self.params):
            result['validate_certs'] = self.params['validate_certs']
        elif self.check_key('F5_VALIDATE_CERTS', os.environ):
            result['validate_certs'] = os.environ['F5_VALIDATE_CERTS']
        else:
            result['validate_certs'] = True

        if self.check_key('auth_provider', provider):
            result['auth_provider'] = provider['auth_provider']
        elif self.check_key('auth_provider', self.params):
            result['auth_provider'] = self.params['auth_provider']
        else:
            result['auth_provider'] = None

        if self.check_key('user', provider):
            result['user'] = provider['user']
        elif self.check_key('user', self.params):
            result['user'] = self.params['user']
        elif self.check_key('F5_USER', os.environ):
            result['user'] = os.environ.get('F5_USER')
        elif self.check_key('ANSIBLE_NET_USERNAME', os.environ):
            result['user'] = os.environ.get('ANSIBLE_NET_USERNAME')
        else:
            result['user'] = None

        if self.check_key('password', provider):
            result['password'] = provider['password']
        elif self.check_key('password', self.params):
            result['password'] = self.params['password']
        elif self.check_key('F5_PASSWORD', os.environ):
            result['password'] = os.environ.get('F5_PASSWORD')
        elif self.check_key('ANSIBLE_NET_PASSWORD', os.environ):
