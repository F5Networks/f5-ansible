...
options:
  ...
  accept_eula:
    description:
      - Specifies that you accept the EULA that is part of iWorkflow. Note
        that this is required to activate the license. If this is not
        specified, or it is set to C(no), then the device will remain in a
        state of limbo until you choose to accept the EULA. This option is
        required when updating a license. It is also suggested that you
        provide it when creating a license, but if you do not, the license
        will remain inactive and you will have to run this module again with
        this option set to C(yes) to activate it.
    default: 'no'
    choices:
      - yes
      - no
  add_on_keys:
    description:
      - A list of additional registration keys.
...
class ModuleManager(object):
    ...
    def _is_unlicensed_eula_unaccepted(self, current):
    ...
    def exists(self):
        result = self.client.api.tm.shared.licensing.registration.load()
        return False
