...
options:
  state:
    description:
      - The virtual server state. If C(absent), deletes the virtual server
  ...
  type:
    description:
      ...
      - When C(stateless), specifies a virtual server that accepts traffic matching the virtual
        ...
        not suitable for processing traffic requiring stateful tracking, such as TCP traffic.
      ...
      - When C(internal), specifies a virtual server that supports modification of HTTP requests
        and responses. Internal virtual servers enable the use of ICAP (Internet Content Adaptation
  ...
  port:
    description:
      - Port of the virtual server. Required when C(state) is C(present)
        and the virtual server does not exist.
      - If you do not want to specify a particular port, use the value C(0).
        This means the virtual server listens on any port.
  ...
  profiles:
    description:
      ...
      - If you want to remove a profile from the list of profiles currently active
        on the virtual, simply remove it from the C(profiles) list. See
        examples for an illustration of this.
      - If you want to add a profile to the list of profiles currently active
        on the virtual, simply add it to the C(profiles) list. See
        examples for an illustration of this.
      - B(Profiles are important). This module will fail to configure a BIG-IP if you mix up
        your profiles, or if you attempt to set an IP protocol which your current,
        or new, profiles do not support. Both this module, and BIG-IP, will report an error
    ...
    suboptions:
      name:
        description:
          ...
          - If this is not specified, it is assumed the profile item is
  ...
  irules:
    description:
      ...
      - The order in which iRules are specified does matter, so a list that contains
        the same list elements but in a different order in the playbook will make changes
        on the device.
  ...
  enabled_vlans:
    description:
      - List of VLANs to enable. When a VLAN named C(all) is used, all
        ...
        the C(partition) option of this module is used.
  ...
  pool:
    description:
      ...
      - When creating a new virtual server, and C(type) is C(stateless), this parameter
        is required.
      - If C(type) is C(stateless), the C(pool) must not have any members
  ...
  snat:
    description:
      ...
      - When C(type) is C(dhcp), C(reject), or C(internal), this parameter is ignored.
      - The name of a SNAT pool (like "/Common/snat_pool_name") can be specified to enable SNAT
  ...
  port_translation:
    description:
      - When C(enabled), specifies the system translates the port of the virtual
        server.
      - When C(disabled), specifies the system uses the port without translation.
  ...
  mask:
   description:
      ...
      - If you specify a catchall destination (for example, C(0.0.0.0) for IPv4, C(::) for IPv6)
        the mask parameter is set to C(any) or C(any6) respectively.
      - When the C(destination) is not in CIDR notation and a C(mask) is not specified, C(255.255.255.255) or
        C(ffff:ffff:ffff:ffff:ffff:ffff:ffff:ffff) is set for IPv4 and IPv6 addresses respectively.
      - When C(destination) is provided in CIDR notation format and a C(mask) is not specified, the mask parameter is
        inferred from C(destination).
      - When C(destination) is provided as a virtual address name, and a C(mask) is not specified,
  ...
  ip_protocol:
    description:
      ...
      - The value of this argument can be specified in either its numeric value, or in a select number
        of named values. Refer to C(choices) for examples.
  ...
  security_log_profiles:
    description:
      ...
      - The C(Log all requests) and C(Log illegal requests) are mutually exclusive and
        therefore, this module raises an error if the two are specified together.
    type: list
    elements: str
  security_nat_policy:
    description:
      ...
      - The most specific policy is used. For example, if you specify the
        virtual server should use the device policy and the route domain policy, the route
    ...
    suboptions:
      ...
      use_device_policy:
        description:
          - Specifies the virtual server uses the device NAT policy, as specified
      ...
      use_route_domain_policy:
        description:
          - Specifies the virtual server uses the route domain policy, as
  ...
  ip_intelligence_policy:
    description:
      ...
      - This parameter requires a valid BIG-IP security module is provisioned,
        such as ASM or AFM.
    type: str
  rate_limit:
    description:
      - Virtual server rate limit (connections-per-second). Setting this to C(0)
        disables the limit.
    ...
    suboptions:
      pool_name:
        description:
          ...
          - Referencing a pool on the Common partition needs to be done in the full path format,
  ...
  check_profiles:
    description:
      - Specifies whether the client and server SSL profiles specified by the user should be verified to be
        correct against the existing profiles. This is useful in cases where a large number of profiles
  ...
  bypass_module_checks:
    description:
      - Disables all built-in module verification checks that require BIG-IP device calls. Using this option cuts
        down on the number of REST calls made by this module. The trade off is that most parameters are sent as is,
